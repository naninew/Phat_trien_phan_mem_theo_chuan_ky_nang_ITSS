...
def create_navbar():
    ...
    with ui.header().classes('glass-panel px-6 py-2 flex items-center justify-between fixed top-0 w-full z-50'):
        # Left side: Logo
        with ui.row().classes('items-center gap-2 cursor-pointer').on('click', lambda: ui.navigate.to('/')):
            ui.icon('local_taxi', size='2rem').classes('text-primary')
            ui.label(APP_TITLE).classes('text-xl font-bold text-primary font-outfit hide-on-mobile')

        # Right side: Auth/User Actions
        with ui.row().classes('items-center gap-4'):
            if is_authenticated():
                # Notifications
                ui.button(icon='notifications').props('flat round').classes('text-on-surface-variant')
                
                # User Profile Dropdown
                with ui.button(get_user_name(), icon='account_circle').props('flat').classes('text-on-surface font-semibold'):
