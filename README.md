...
│   │   │   ├── user.py      # → UC001, UC002, UC012
│   │   │   ├── company.py   # → UC013, UC014
│   │   │   ├── request.py   # → UC003~UC008, UC011
│   │   │   ├── vehicle.py   # → UC010
│   │   │   ├── payment.py   # → UC007
│   │   │   ├── review.py    # → UC008, UC015
│   │   │   └── community.py # → UC009, UC015
│   │   ├── schemas/         #  Validation (Pydantic)
│   │   │   ├── auth.py      # Rules mật khẩu, email, SĐT
│   │   │   └── rescue.py    # Rules GPS, ETA, biển số, trạng thái
│   │   ├── services/        # -- Business Logic (thuần Python)
│   │   │   ├── auth_svc.py  # Hash pwd, JWT sign, role check
│   │   │   ├── rescue_svc.py# Haversine calc, status transition, ETA
│   │   │   ├── payment_svc.py# Mock/Real VNPay/MoMo integration
│   │   │   └── report_svc.py# Aggregation queries for UC016
│   │   ├── controllers/     #  Nhận request, gọi service, trả JSON
│   │   ├── routes/          # -- Blueprint routers
│   │   │   ├── auth_routes.py    # /api/v1/auth/*
│   │   │   ├── rescue_routes.py  # /api/v1/rescue/*
│   │   │   ├── admin_routes.py   # /api/v1/admin/*
│   │   │   └── payment_routes.py # /api/v1/payment/*
│   │   └── utils/
│   │       ├── jwt_helper.py
│   │       └── response.py       # Chuẩn hóa {code, msg, data}
│   └── run.py
│
└── frontend/                 # -- NiceGUI (Python-only)
    ├── main.py               # -- Entry point, đăng ký @ui.page(), ui.run()
    ├── core/
    │   ├── config.py         # BACKEND_URL="http://localhost:5000/api/v1"
    │   └── auth.py           # Wrapper app.storage.user (token, role, profile)
    ├── pages/
    │   ├── auth/
    │   │   ├── login.py      # → UC001
    │   │   └── register.py   # → UC002
    │   ├── customer/
    │   │   ├── dashboard.py  # Layout chung + menu điều hướng
    ...
    │   ├── company/
    │   │   ├── dashboard.py
    │   │   ├── queue.py      # → UC011 (tiếp nhận/điều phối)
    │   │   ├── fleet.py      # → UC010
    │   │   └── profile.py    # → UC014
    │   └── admin/
    │       ├── dashboard.py
    ...
    │       ├── moderation.py # → UC015
    │       └── reports.py    # → UC016
    ├── components/           # -- UI tái sử dụng (không phụ thuộc page)
    │   ├── navbar.py         # Drawer + Role-based menu
    │   ├── map_view.py       # Leaflet embed (tìm kiếm/theo dõi)
    │   ├── status_timeline.py# Hiển thị tiến độ cứu hộ
    │   ├── chat_box.py       # Tư vấn cộng đồng / hỗ trợ
    │   └── payment_form.py   # Form chọn phương thức + QR
    ├── services/
    │   ├── api_client.py     # httpx.AsyncClient base (retry, timeout)
    │   ├── auth_api.py       # login, register, refresh_token
    │   ├── rescue_api.py     # find, send, track, cancel, update_status
    │   ├── payment_api.py    # create_payment, verify_callback
    │   └── admin_api.py      # CRUD users/companies, moderate, export
    └── static/               # -- logo, icons, custom CSS



USER FLOW CHUẨN HÓA

# Luồng 1: NGƯỜI DÙNG CÁ NHÂN (Customer)
Mở App → Tự động redirect đến /login (UC001). Nếu chưa có tài khoản → /register (UC002).
Vào Dashboard → Chọn "Tìm cứu hộ gần đây" (UC003).
Nhập loại sự cố (xì lốp, chết máy...) → Hệ thống lấy GPS → Hiển thị danh sách công ty trong bán kính R km (khoảng cách, giá dự kiến, ETA, rating).
Chọn công ty → Chuyển sang form "Gửi yêu cầu" (UC004). Auto-fill GPS, thời gian. Người dùng nhập mô tả, đính kèm ảnh (tùy chọn), chọn phương thức thanh toán ưu tiên → Nhấn Gửi.
Chuyển trạng thái Chờ tiếp nhận → Vào trang "Theo dõi" (UC005). Xem timeline: Đã gửi → Đã tiếp nhận → Đang di chuyển → Đang xử lý → Hoàn thành.
(Tùy chọn) Trong trạng thái Chờ tiếp nhận → Nhấn "Huỷ" (UC006) → Xác nhận → Cập nhật Đã huỷ.
Khi hoàn thành → Vào trang "Thanh toán" (UC007). Chọn phương thức (Tiền mặt/MoMo/VNPay/Thẻ) → Xử lý qua API cổng thanh toán → Nhận biên lai.
Mở khóa nút Đánh giá → Vào "Đánh giá" (UC008). Chấm sao 1-5, nhập nhận xét → Gửi.
(Tùy chọn) Vào "Cộng đồng" (UC009) → Đăng bài hỏi đáp sự cố, xem/react comment realtime.

# Luồng 2: CÔNG TY CỨU HỘ (Rescue Company)
Login → Vào Dashboard Công ty → Xem danh sách yêu cầu mới (Chờ tiếp nhận).
Chọn yêu cầu → Xem chi tiết (vị trí, loại sự cố, ảnh) → Nhấn "Tiếp nhận" hoặc "Từ chối".
Nhập ETA (thời gian dự kiến đến) → Hệ thống gửi notification tới người dùng.
Điều phối → Chọn nhân viên & phương tiện từ danh sách Sẵn sàng (UC010) → Gán vào yêu cầu.
Cập nhật trạng thái (UC011): Nhân viên/Admin công ty bấm cập nhật Đang di chuyển → Đang xử lý → Hoàn thành. Hệ thống sync realtime sang app người dùng.
Quản lý đội xe (UC010): Thêm/sửa/xóa phương tiện (biển số, loại, trọng tải). Không được xóa xe Đang làm nhiệm vụ.
Cập nhật thông tin công ty (UC014): Địa chỉ, phạm vi hoạt động, dịch vụ cung cấp.
# Luồng 3: QUẢN TRỊ VIÊN (Admin)
Login → Vào Admin Dashboard → Xem tổng quan hệ thống (số yêu cầu, công ty active, doanh thu).
Quản lý người dùng (UC012): CRUD tài khoản khách hàng, khóa/mở khóa tài khoản vi phạm.
Quản lý công ty (UC013): Duyệt đăng ký công ty mới, xác minh giấy phép, khóa tài khoản.
Kiểm duyệt nội dung (UC015): Xem danh sách bài đăng cộng đồng & đánh giá bị report → Duyệt hoặc Gỡ bỏ.
Báo cáo (UC016): Chọn khoảng thời gian, loại báo cáo → Xuất Excel/PDF → Xem biểu đồ tần suất, tỷ lệ phản hồi, mức độ hài lòng.
